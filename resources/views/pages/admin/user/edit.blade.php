                        ...
                        <div class="card-body personal-info">
                            <div class="d-flex align-items-center justify-content-between mb-4">
                                <h5 class="fw-bold mb-0 me-4">
                                    <span class="d-block mb-2">Edit User</span>
                                    <span class="fs-12 fw-normal text-muted text-truncate-1-line">Please fill in the
                                        details below:</span>
                                </h5>
                            </div>
                            <form action="{{ route('master.users.update', $user->intUser_ID) }}" method="POST">
                                @csrf
                                @method('PUT')
                                    ...
                                    <div class="col-lg-8">
                                        <div class="input-group">
                                            ...
                                            <select class="form-select" style="font-size: 13.52px" id="txtGender"
                                                ...
                                                <option value="L" {{ $user->txtGender == 'L' ? 'selected' : '' }}>Male
                                                </option>
                                                <option value="P" {{ $user->txtGender == 'P' ? 'selected' : '' }}>
                                                    Female</option>
                                            </select>
                                        </div>
                                    </div>
                                        ...
                                        <div class="input-group">
                                            ...
                                            <select class="form-select" style="font-size: 13.52px" id="intRole_ID"
                                                ...
                                                @foreach ($roles as $role)
                                                    <option value="{{ $role->intRole_ID }}"
                                                        {{ $user->intRole_ID == $role->intRole_ID ? 'selected' : '' }}>
                                                        {{ $role->txtRole }}
                                                    </option>
                                                @endforeach
                                            </select>
                                            ...
                                            <select class="form-select" style="font-size: 13.52px" id="intDepartment_ID"
                                                ...
                                                @foreach ($departments as $department)
                                                    <option value="{{ $department->intDepartment_ID }}"
                                                        {{ $user->intDepartment_ID == $department->intDepartment_ID ? 'selected' : '' }}>
                                            ...
                                            <select class="form-select" style="font-size: 13.52px" id="intGroup_ID"
                                                ...
                                                @foreach ($groups as $group)
                                                    <option value="{{ $group->intGroup_ID }}"
                                                        {{ $user->groups->contains($group->intGroup_ID) ? 'selected' : '' }}>
                                                        {{ $group->txtGroupName }}
                                                    </option>
                                                @endforeach
                                            </select>
